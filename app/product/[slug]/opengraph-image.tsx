...
import { productsBySlug, products } from "@/lib/index";
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const title = product ? `${product.product} default credentials` : "Default Credentials";
  const sub = product
    ? [
        product.vendor && `Vendor: ${product.vendor}`,
        `${product.credentials.length} credential set${product.credentials.length === 1 ? "" : "s"}`,
      ]
        .filter(Boolean)
        .join("  ·  ")
    : "";
